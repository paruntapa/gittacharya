    ...
    <div className="max-w-[70vw] ">
        <Tabs value={tab} onValueChange={setTab}>
          <div className="overflow-x-scroll flex gap-2 bg-gray-200 p-1 rounded-md">
            {filesReferences.map(file=>(
              <button onClick={() => setTab(file.fileName)} key={file.fileName} className={cn(
                'px-2 py-1.5 text-sm font-medium rounded-md transition-colors whitespace-nowrap text-muted-foreground hover:bg-muted hover:text-black',
                {
          ...
          {filesReferences.map(file => (
            ...
            <TabsContent key={file.fileName} value={file.fileName} className='max-h-[60vh] overflow-scroll max-w-7xl rounded-md'>
